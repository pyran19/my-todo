...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    lifecycle_locked INTEGER NOT NULL DEFAULT 0         -- 手動移動済みフラグ (0/1)
);
```

- `lifecycle`: 短期=`short` / 中期=`mid` / 長期=`long`。
- `status`: `open`（未完了） / `done`（完了）。
- `lifecycle_locked`: 1 のとき、自動移行（第4節）の対象外。`promote` / `demote` で手動移動したタスクに立てる。
- 日付はすべてアプリ側で自動設定（F-2）。ユーザーは入力しない。
- 既存DBに `lifecycle_locked` が無い場合は、接続時に `ALTER TABLE` で冪等に追加する（軽量マイグレーション）。
...
| 移行 | 条件（経過日数の目安） |
|------|------------------------|
| `short` → `mid` | 作成から約 **7日** 経過 |
| `mid` → `long`  | 作成から約 **30日** 経過 |
...
2. `status = 'open'` かつ `lifecycle_locked = 0` のタスクのうち、`created_at` からの経過日数がしきい値を超えるものについて `lifecycle` を1段階昇格し、`promoted_at` と `updated_at` を更新する。
3. 1回のチェックで複数段階の昇格条件を満たす場合（例: 久しぶりの実行で30日以上経過）、最終的に到達すべき段階まで一度に昇格させる。
...
- 手動移動したタスクは `lifecycle_locked = 1` とし、以降の自動移行で上書きされないようにする（特に `demote` が次回実行の自動昇格で即座に巻き戻るのを防ぐ）。手動操作を自動より優先する方針。
...
| `todo promote <id>` | タスクを一つ長寿命側の段階へ手動移動（`short`→`mid`→`long`）。手動移動フラグを立てる |
| `todo demote <id>` | タスクを一つ短寿命側の段階へ手動移動（`long`→`mid`→`short`）。手動移動フラグを立てる |
...
`todo ls` は `short` / `mid` / `long` の見出しごとにタスクをまとめて表示する。各行に `id`・本文・（任意で）作成からの経過日数を示し、寿命がひと目で分かるようにする。手動移動済み（`lifecycle_locked = 1`）のタスクには 🔒 を付す。
